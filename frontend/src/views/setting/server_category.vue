...
<div class="block" style="float:right;margin-top:20px ;margin-right:280px" >
  ...
  <el-dialog title="增加应用类型"   width="30%"  :visible.sync="dialogFormVisible">
    <el-form :model="addform"   :rules="addformRules"    :label-position="labelPosition"   >
      ...
      <el-form-item label="应用标识"  :label-width="formLabelWidth" >
        <el-input type='text'  size='mini' v-model="addform.application_tag"   ></el-input>
      </el-form-item>
      <el-form-item   label="产品线"   :label-width="formLabelWidth" >

      <el-select   v-model="addform.nameinfo" laceholder="请选择">
          <el-option
           v-for="item in  addform.line_names"
            :key="item.id"
  ...
  <el-dialog title="编辑应用"    width="30%"   :visible.sync="editlogFormVisible">
    <el-form    :model="editform"  >
      ...
      <el-form-item label="产品线"  :label-width="formLabelWidth" >
        <el-select v-model="editform.line_name"  filterable placeholder="请选择">
      <el-option
       v-for="item in  addform.line_names"
        :key="item.id"
        :label="item.line_name"
  ...
  export default {
    ...
    data () {
      return {
        ...
        addformRules: {
        application_name:[{ required: true, message: "应用名称！", trigger: "blur" }],
        application_tag: [{ required: true, message: 'title is required', trigger: 'blur' }]
      },
    ...
    methods:{
      ...
      filterData2(data){
        ...
        for (var i=0;i<data.results.length;i++){
          ...
          linefinaldata.push(dataobj);
        }
        this.addform.line_names = linefinaldata;
    ...
    //获取应用列表
      ...
      filterData(data){
        ...
        for (var i=0;i<data.results.length;i++){
          var dataobj ={
            ...
            line_name:data.results[i].line_name,
            line_idname:data.results[i].line_idname,
      ...
      addinfo(){
        this.$axios.post("http://127.0.0.1:8000/api/applications/", {
          ...
          line_name: this.addform.nameinfo,
          ...
          .then(res => {
            if (res.status === 201){
              ...
              //失败
              this.$message({message:res.re,type:'error'});
            }
      ...
      if (res.status === 200 ) {
         ...
         this.editform ={
          ...
          line_idname:res.data.line_idname,
           };
